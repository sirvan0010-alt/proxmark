...
#pragma warning disable CS0067 // Interface event is intentionally unused by this synchronous simulator.
...
#pragma warning restore CS0067
    ...
    public uint SysTimestamp { get; set; }
    ...
    {
        cancellationToken.ThrowIfCancellationRequested();
        if (!IsConnected) throw new InvalidOperationException("Simulated BWM transport is not connected.");
        if (Fault == SimulationFault.DisconnectBeforeSend) { IsConnected = false; throw new InvalidOperationException("Simulated connection loss before send."); }
        if (Fault == SimulationFault.Timeout) throw new TimeoutException("Simulated BWM timeout.");
        if (!BwmFrameCodec.TryDecode(request.Span, out var frame) || frame is null || frame.Kind != BwmFrameKind.Request) return Task.FromResult(Array.Empty<byte>());
        var command = (BwmCommandCode)frame.CommandId;
        if (Fault == SimulationFault.UnsupportedCommand) return Task.FromResult(Array.Empty<byte>());
        ...
        if (Fault == SimulationFault.MalformedResponse) return Task.FromResult(new byte[] { 0x00, 0xFF, 0x00 });
        ushort responseCommand = frame.CommandId;
        if (Fault == SimulationFault.WrongCommandId) responseCommand = unchecked((ushort)(responseCommand + 1));
